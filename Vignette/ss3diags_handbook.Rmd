...
To visiualize the runs test for multiple indices, it is recommended to use of the function `r4ss::sspar()`. The option plot `add=TRUE` in `sspar()` facilitates setting the graphic parameters so that they are suitable for `ss3diags` plots. The option `add=TRUE` prevents the plotting functions from over-writing `sspar()`.
