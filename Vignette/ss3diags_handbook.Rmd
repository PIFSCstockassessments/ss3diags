...
```{r fig7, fig.cap= "Hindcasting cross-validation (HCxval) results for mean lengths. Note that MASE values in brackets are adjusted MASE values for cases where naive predictions have a Mean-Absolute-Error below 0.1"}
...
Below is a step-by-step cookbook recipe for retrospective analysis in Stock Synthesis which can also be found in the [model recipes](https://github.com/PIFSCstockassessments/ss3diags/blob/main/model_recipes/R0_likelihood_profiling.r).  
