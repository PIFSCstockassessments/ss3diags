...
file.copy(from ="../../inst/extdata/ss.exe", to = dir_tmp)
file.copy(from ="../../inst/extdata/ss.exe", to = dir_aspm)
```


You will need to make sure you have the [SS executable](https://github.com/nmfs-stock-synthesis/stock-synthesis) file either in your path or in the directory you are running the ASPM from (in this case `dir_aspm`). 
...
```{r}
r4ss::run(dir = dir_tmp, verbose = FALSE, exe = "ss.exe")
files <- c("data_echo.ss_new", "control.ss_new", "starter.ss", "forecast.ss", "ss.par")
...
```{r}
r4ss::run(dir = dir_aspm, exe = "ss.exe", skipfinished = FALSE, verbose = FALSE)
```
